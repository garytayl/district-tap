...
export default function NorthsideSpacePage({ params }: { params: { slug: string } }) {
  const space = northsideSpaces.find((item) => item.slug === params.slug)
  ...
  return (
    <main className="mx-auto flex max-w-4xl flex-col gap-10 px-6 py-16">
      <PageHeader
        eyebrow="Northside Private Event"
        title={space.name}
      ...
      <Card>
        ...
        {space.notes ? (
          <ul className="mt-3 list-disc space-y-1 pl-5 text-xs text-black/50">
